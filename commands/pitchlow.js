...
module.exports = {
    ...
    run: async ({ sock, msg }) => {
        ...
        if (!ctx?.quotedMessage?.audioMessage) {
            return sock.sendMessage(jid, { text: "❌ Reply to an audio with `.pitchlow`" })
        }
        ...
        try {
            ...
            exec(
                `ffmpeg -y -i "${input}" -af "rubberband=pitch=0.8" "${output}"`,
                async (err) => {
                    if (err) throw err
            ...
            console.error("PITCH LOW ERROR:", e)
