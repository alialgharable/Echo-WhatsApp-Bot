...
module.exports = {
    name: "grayscale",
    description: "Convert image to black & white using FFmpeg",
    ...
    run: async ({ sock, msg }) => {
        ...
        const inputPath = path.join(tempDir, `gray_in_${id}.png`)
        const outputPath = path.join(tempDir, `gray_out_${id}.jpg`)
        ...
        try {
            ...
            await new Promise((res, rej) => {
                exec(
                    `"${FFMPEG}" -y -i "${inputPath}" -vf "format=gray" "${outputPath}"`,
                    err => err ? rej(err) : res()
                )
            })
